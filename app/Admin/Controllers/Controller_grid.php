...
use Components\Model\Model as ModelBase;
...
class Controller extends BaseController
{
    ...
    {
        $class = 'Components\Model\\' . ucfirst($this->router->getControllerName());
    ...
    private function delete($id, $model = null)
    {
        ...
        if (empty($model)) {
            ...
            $class = 'Components\Model\\' . ucfirst($model);
        }
        if (!class_exists($class)) {
            return false;
        }
    ...
    public function paginatorQueryBuilder($model, $page)
    {
        $builder = $this->modelsManager->createBuilder()
            ->from('Components\\Model\\' . $model['name'])
    ...
    protected function renderGrid($class)
    {
        ...
        */
        ...
        $builder->from(['a' => 'Components\\Model\\' . $class]);
        if (!empty(self::$grid['query'])) {
            ...
            foreach (self::$grid['query']['joins'] as $join) {
                ...
                if (in_array($type, ['innerJoin', 'leftJoin', 'rightJoin', 'join'])) {
                    $builder->$type('Components\\Model\\' . $join['model'], $join['on'], $join['alias']);
                }
            }
        ...
        if (!count($collections)) {
            if (!$this->request->isAjax()) {
                $this->flashSession->notice("The search did not match any collection.");
            } else {
                /*
