        ...
        <table class="table b-t b-light table-condensed table-hover">
            ...
            <tr>
                ...
                <th width="135">
                    <div class="btn-group">
                        {% if filters is empty %}
                            {{ submit_button('Filter', 'class' : 'btn btn-sm btn-success auto', 'id' : 'filter', 'name' : 'filter') }}
                        {% else %}
                            {{ submit_button('Filter', 'class' : 'btn btn-sm btn-success auto', 'id' : 'filter', 'name' : 'filter') }}
                        ...
                        {% elseif row[method] is defined %}
                            <td>
                                {% if grid['grid'][method]['tooltip'] is defined %}
                                    <span data-toggle="tooltip" data-original-title="{{ row[method] | trim | striptags }}" data-placement="bottom">{{ row[method] | trim | striptags  }} </span>
                                {% elseif grid['grid'][method]['view'] is defined %}
                                    {{ link_to(this.view.getControllerName() | lower ~ '/view/' ~ row['id'] | trim, row[method] | trim | striptags , 'title' : row[method] | trim | striptags ) }}
    ...
    <footer class="panel-footer">
        <div class="row">
            ...
            <div class="col-sm-3 text-right text-center-xs">
                {% include "admin/partials/pagination.volt" %}
            </div>
        </div>
    </footer>
    {{ endform() }}
